...
class groupAttachments extends StatefulWidget {
  String ID;
  groupAttachments(this.ID , {Key key}) : super(key:key);
  @override
  _groupAttachmentsState createState() => _groupAttachmentsState();
}

class _groupAttachmentsState extends State<groupAttachments> with SingleTickerProviderStateMixin {
  ...
  Future getDetails(String chatID) async{
    await Firestore.instance.collection('groupChats').document(chatID).get().then((val){
      setState(() {
              ...
              child: Container(
                child: new TabBarView(
                  ...
                  children: <Widget>[
                    new Container(
                        child: StreamBuilder(
                          stream: Firestore.instance.collection("groupChats").
                          document(widget.ID).collection("chats").where('message type',isEqualTo: "image").
                          orderBy('time',descending:true).snapshots(),
                          builder: (context , snapshot){
                            if(!snapshot.hasData){
                              return Center(child: Text("No Images" , style: TextStyle(
                                fontFamily: "Poppins"
                              ),));
                            }else{
                    ...
                    new Container(
                        child: StreamBuilder(
                            ...
                            builder: (context , snapshot){
                              if(!snapshot.hasData){
                                return Center(child: Text("No Videos" , style: TextStyle(
                                    fontFamily: "Poppins"
                                ),));
                              }else{
                    ...
                    new Container(
                        child:StreamBuilder(
                          ...
                          builder: (context , snapshot){
                            if(!snapshot.hasData){
                              return Center(child: Text("No Files" , style: TextStyle(
                                  fontFamily: "Poppins"
                              ),));
                            }else{
                              return ListView.separated( // show the list of files

...
import 'package:alpinemessenger/screens/chatInfo.dart';
...
class chatScreen extends StatefulWidget {
  ...
  chatScreen(this.name,this.Receiveremail, this.chatRoomId, this.display , {Key key}) : super(key: key);
  @override
...
class _chatScreenState extends State<chatScreen> {
    ...
    await ref.putFile(file).onComplete.then((value){
      if(value.error == null){
        ...
        storageTaskSnapshot.ref.getDownloadURL().then((downloadURL){
            addMessagesoftheConvo(widget.chatRoomId,widget.Receiveremail, downloadURL, messageType);
        });
      }else{
  ...
  Widget getMessages(){
    return StreamBuilder(
      stream: Firestore.instance.collection('chatRoom').document(widget.chatRoomId).collection('chats').orderBy('time',descending: true).snapshots(),
          ...
          return ListView.builder(
            ...
            itemBuilder: (context , index){
              return ChatMessage(
                ...
                sentByMe: (globals.email == snapshot.data.documents[index].data['sentBy']) ? true : false,
          ...
          IconButton(
            ...
            onPressed: (){
              if(_messageController.text.isEmpty){
                ...
                addMessagesoftheConvo(widget.chatRoomId, widget.Receiveremail, _messageController.text , 'text');
          ...
          1.1 , AppBar(
              ...
              child: ListTile(
                onTap: (){
                  Navigator.push(
                    ...
                    builder: (context) => new chatInfo(widget.chatRoomId)
                  )
                  );
                },
                contentPadding: EdgeInsets.all(0.0),
                leading: (widget.display == null) ?  CircleAvatar(
                  radius: 18,
                  backgroundColor: Colors.white,
                  child: Text(widget.name[0] , style: TextStyle(
                    color: Colors.blue[800] , fontSize: 16.0
                  ),),
                ) : CircleAvatar(
                  radius: 18,
                  backgroundImage: NetworkImage(widget.display),
                ),
            ...
            actions: <Widget>[
              Padding(
                ...
                child: IconButton( icon : Icon(Icons.videocam, color: Colors.white,size: 30.0,)),
              ) ,
            ],
          ),
        ),
...
class ChatMessage extends StatelessWidget {
  ...
  ChatMessage({this.text , this.sentByMe , this.time , this.messageType});

  @override
  Widget build(BuildContext context) {
    return Container(
      alignment:(sentByMe) ? Alignment.centerRight : Alignment.centerLeft,
      margin:(sentByMe) ? EdgeInsets.only(top: 8.0 , bottom: 10.0 , left: 90.0 , right: 5.0)
        : EdgeInsets.only(top: 8.0 , bottom: 10.0 , left: 5.0 , right: 90.0) ,
      child: (messageType == 'text') ? Column(
